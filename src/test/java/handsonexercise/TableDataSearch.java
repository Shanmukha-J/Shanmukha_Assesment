...
public class TableDataSearch {
	...
	public void TableDataSearchTest() {
		String expected = "in";
		WebElement ele = driver.findElement(By.id("task-table-filter"));
		ele.clear();
		...
		for(WebElement oElem : listTotalRows) {
			String atrStyle=oElem.getAttribute("style");
			if(!atrStyle.equalsIgnoreCase("display: none;")) {
				String actual = oElem.getText();
				sf.assertTrue(actual.contains(expected), "FAIL| actual "+actual+" expected "+ expected);
			}
		}
		sf.assertAll();
	}
